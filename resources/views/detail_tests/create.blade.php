...
<div class="page-inner" style="margin-top: 2.5cm">
    <div class="card shadow mb-4">
        ...
        <div class="card-body" style="margin-bottom: 2cm">
            <form action="{{ route('detail_tests.store') }}" method="POST">
                @csrf
                <div class="row mb-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" class="form-control @error('urutan') is-invalid @enderror" id="urutan"
                            name="urutan" value="{{ old('urutan') }}" min="0" required>
                        @error('urutan')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div id="nilai-normal-container">
                    @if(old('nilai_normals_data'))
                    @foreach(old('nilai_normals_data') as $index => $data)
                            ...
                            <div class="row">
                                <div class="col-md-2">
                                    ...
                                    <select
                                        ...
                                        <option value="Umum" {{ old('nilai_normals_data.' . $index . '.jenis_kelamin' ,
                                            isset($data['jenis_kelamin']) ? $data['jenis_kelamin'] : '' )=='Umum'
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="number"
                                        ...
                                        value="{{ old('nilai_normals_data.' . $index . '.usia_min', isset($data['usia_min']) ? $data['usia_min'] : '') }}"
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="number"
                                        ...
                                        value="{{ old('nilai_normals_data.' . $index . '.usia_max', isset($data['usia_max']) ? $data['usia_max'] : '') }}"
                                        min="0" style="background-color: white; border-radius: 6px;">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select
                                        ...
                                        <option value="Single" {{ old('nilai_normals_data.' . $index . '.type' ,
                                            isset($data['type']) ? $data['type'] : '' )=='Single' ? 'selected' : '' }}>
                                            Single</option>
                                        <option value="Range" {{ old('nilai_normals_data.' . $index . '.type' ,
                                            isset($data['type']) ? $data['type'] : '' )=='Range' ? 'selected' : '' }}>
                                            Range</option>
                                    </select>
                                ...
                                <div class="col-md-3">
                                    <div class="row" id="range-fields-{{ $index }}"
                                        style="display: {{ old('nilai_normals_data.' . $index . '.type', isset($data['type']) ? $data['type'] : '') == 'Range' ? 'flex' : 'none' }}; align-items: flex-end;">
                                        <div class="col-md-6">
                                            ...
                                            <input type="number" step="0.01"
                                                ...
                                                value="{{ old('nilai_normals_data.' . $index . '.min', isset($data['min']) ? $data['min'] : '') }}"
                                                style="background-color: white; border-radius: 6px;">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="number" step="0.01"
                                                ...
                                                value="{{ old('nilai_normals_data.' . $index . '.max', isset($data['max']) ? $data['max'] : '') }}"
                                                style="background-color: white; border-radius: 6px;">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.nilai-normal-row').forEach(row => {
            const idParts = row.id.split('-'); // e.g., "nilai-normal-row-0"
            const index = parseInt(idParts[idParts.length - 1]);
            if (!isNaN(index) && index >= nilaiNormalIndex) {
                nilaiNormalIndex = index + 1;
            }
        });
        if (nilaiNormalIndex === 0 && !document.querySelector('.nilai-normal-row')) {
            nilaiNormalIndex = 0;
        }
        ...
        document.querySelectorAll('.nilai-normal-row .remove-nilai-normal-btn').forEach(function(button) {
