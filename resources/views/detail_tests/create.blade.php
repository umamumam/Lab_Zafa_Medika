        ...
        <div class="card-body" style="margin-bottom: 2cm">
            <form action="{{ route('detail_tests.store') }}" method="POST">
                ...
                <div class="form-group mt-4">
                    ...
                    <a href="{{ route('detail_tests.index', $test->id) }}" class="btn btn-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
...
{{-- <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        if (document.getElementById('type').value === 'Range') {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (typeSelect.value === 'Range') {
            rangeFields.style.display = 'flex'; // Changed to flex for better alignment
            document.getElementById('min').required = true;
            document.getElementById('max').required = true;
        }
